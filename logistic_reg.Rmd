...
```{r}
fit.1 = glm(death_update ~ formersmoker  ,data =mi_df ,family = 'binomial')
...
# option 2 
b1 + c(-1,1) * qnorm(.975) * summary(fit.1)$coefficient[2,2]
```
...
```{r}
fit.2 =  glm(death_update ~ factor(Sex)  ,data =mi_df ,family = 'binomial')
summary(fit.2)
```
